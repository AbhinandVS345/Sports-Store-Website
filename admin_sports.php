...
if(isset($_POST["submit"])) {
    ...
    if (isset($_POST['name'])){
        $sportsName = $_POST['name'];
        /* $sportsImage=$_FILES['image']['name'];
        // Move uploaded file to desired location
        move_uploaded_file($_FILES['image']['tmp_name'], 'uploads/' . $sportsImage); */
        // Insert into database
        $sql = "INSERT INTO sports (name) VALUES ('$sportsName')";
        // Execute SQL query
        if(mysqli_query($conn, $sql)) {
         $_SESSION['status'] = "Sports added successfully!";
        } else {
         $_SESSION['status'] = "Sports addition failed!";
        }
     
    
     }
     else{
        echo "sports value is not set or empty.";
     }
    }elseif (isset($_POST["delete"])){
       
      $sports_id=$_POST['delete'];
      $sql="DELETE FROM sports WHERE sports_id='$sports_id'";
      if(mysqli_query($conn, $sql)) {
        $_SESSION['status'] = "sports deleted successfully!";
       } else {
        $_SESSION['status'] = "sports deletion failed!";
       }
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center space-x-4 text-sm">
                    <form method="POST" action="">
                      <input type="hidden" name="delete" value="<?php echo $row['sports_id']; ?>">
